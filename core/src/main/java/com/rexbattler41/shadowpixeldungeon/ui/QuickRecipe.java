...
package com.rexbattler41.shadowpixeldungeon.ui;
...
import com.rexbattler41.shadowpixeldungeon.items.food.*;
...
import com.rexbattler41.shadowpixeldungeon.items.potions.brews.BlizzardBrew;
import com.rexbattler41.shadowpixeldungeon.items.potions.brews.CausticBrew;
import com.rexbattler41.shadowpixeldungeon.items.potions.brews.InfernalBrew;
import com.rexbattler41.shadowpixeldungeon.items.potions.brews.ShockingBrew;
import com.rexbattler41.shadowpixeldungeon.items.potions.elixirs.*;
...
import com.rexbattler41.shadowpixeldungeon.items.spells.*;
...
import com.rexbattler41.shadowpixeldungeon.windows.WndInfoItem;
...
public class QuickRecipe extends Component {
	...
	public QuickRecipe(Recipe r, ArrayList<Item> inputs, final Item output) {
		...
		for (final Item in : inputs) {
			...
			curr = new ItemSlot(in) {
				...
				protected void onClick() {
					shadowpixeldungeon.scene().addToFront(new WndInfoItem(in));
				}
			};
		...
		if (hasInputs) {
			...
			if (!(shadowpixeldungeon.scene() instanceof AlchemyScene)) {
				arrow.enable(false);
			}
		} else {
		...
		this.output = new ItemSlot(output){
			@Override
			protected void onClick() {
				shadowpixeldungeon.scene().addToFront(new WndInfoItem(output));
			}
		};
		if (!hasInputs){
	...
	public class arrow extends IconButton {
		...
		protected void onClick() {
			...
			((AlchemyScene) shadowpixeldungeon.scene()).populate(ingredients, Dungeon.hero.belongings);
		}
