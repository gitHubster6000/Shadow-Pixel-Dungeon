...
import com.rexbattler41.shadowpixeldungeon.ui.CheckBox;
import com.rexbattler41.shadowpixeldungeon.ui.GameLog;
import com.rexbattler41.shadowpixeldungeon.ui.Icons;
import com.rexbattler41.shadowpixeldungeon.ui.OptionSlider;
import com.rexbattler41.shadowpixeldungeon.ui.RedButton;
import com.rexbattler41.shadowpixeldungeon.ui.RenderedTextBlock;
...
public class WndSettings extends WndTabbed {
	...
	public void hide() {
		...
		shadowpixeldungeon.seamlessResetScene(new Game.SceneChangeCallback() {
			@Override
			...
			if (DeviceCompat.isAndroid() && PixelScene.maxScreenZoom >= 2) {
				chkSaver = new CheckBox(Messages.get(this, "saver")) {
					...
					protected void onClick() {
						...
						if (checked()) {
							checked(!checked());
							shadowpixeldungeon.scene().add(new WndOptions(Icons.get(Icons.DISPLAY),
		...
		protected void createChildren() {
			...
			if (Math.min(wMin, hMin) >= 2*Game.density){
				optUIMode = new OptionSlider(
					...
					protected void onChange() {
						...
						shadowpixeldungeon.seamlessResetScene();
					}
				};
			...
			if (SPDSettings.interfaceSize() == 0) {
				btnToolbarSettings = new RedButton(Messages.get(this, "toolbar_settings"), 9){
					@Override
					protected void onClick() {
						shadowpixeldungeon.scene().addToFront(new Window(){
		...
		protected void createChildren() {
			...
			if (DeviceCompat.hasHardKeyboard()){
				...
				btnKeyBindings = new RedButton(Messages.get(this, "key_bindings")){
					...
					protected void onClick() {
						super.onClick();
						shadowpixeldungeon.scene().addToFront(new WndKeyBindings(false));
					}
				};
			...
			if (ControllerHandler.isControllerConnected()){
				btnControllerBindings = new RedButton(Messages.get(this, "controller_bindings")){
					...
					protected void onClick() {
						super.onClick();
						shadowpixeldungeon.scene().addToFront(new WndKeyBindings(true));
					}
				};
			...
			if (currLang != Languages.ENGLISH) {
				...
				btnCredits = new RedButton(credText, credText.length() > 9 ? 6 : 9) {
					...
					protected void onClick() {
						...
						shadowpixeldungeon.scene().addToFront(credits);
					}
				};
				add(btnCredits);
			}
