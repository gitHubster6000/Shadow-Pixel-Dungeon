...
import com.rexbattler41.shadowpixeldungeon.items.EnergyCrystal;
...
public class WndEnergizeItem extends WndInfoItem {
	...
	public static void energize( Item item ) {
		...
		if (shadowpixeldungeon.scene() instanceof AlchemyScene){
			...
			((AlchemyScene) shadowpixeldungeon.scene()).createEnergy();
	...
	public static WndBag openItemSelector(){
		if (shadowpixeldungeon.scene() instanceof GameScene) {
			...
			shadowpixeldungeon.scene().addToFront(window);
			return window;
		}
	}
		...
		public void onSelect(Item item) {
			if (item != null) {
				...
				if (shadowpixeldungeon.scene() instanceof GameScene) {
					...
					shadowpixeldungeon.scene().addToFront(new WndEnergizeItem(item, parentWnd));
				}
			}
		}
	};
